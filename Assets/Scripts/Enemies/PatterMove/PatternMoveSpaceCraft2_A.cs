...
{
    public class PatternMoveSpaceCraft2_A : IPatternMove
    {
        
        public PatternMoveSpaceCraft2_A(int index)
        {
            _index = index;
        }
        private int _index;
        ...
        public Sequence Move(Transform target, Action OnShoot, Action OnDeactivate, Action OnLookAt)
        {
            ...
            //Initial Position
            target.localPosition = new Vector3((_index % 2 == 0) ? startPosition1.x : -startPosition1.x, startPosition1.y, startPosition1.z);
            target.localRotation = Quaternion.Euler(0, 0, 0);
            //Sequence
            sec.AppendInterval(waitForStart);
            sec.Append(target.DOLocalMove(new Vector3((_index % 2 == 0) ? endPosition1.x : -endPosition1.x, endPosition1.y, endPosition1.z), timeToMove1).SetEase(Ease.OutCirc));
