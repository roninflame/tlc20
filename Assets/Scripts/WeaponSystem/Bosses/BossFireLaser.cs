...
{
    public class BossFireLaser : Laser
    {
        //SHARE
        ...
        [SerializeField] private float _radiusCollider = 30;
        //[SerializeField] private Vector3 _colliderScale = Vector3.one;


        public float LaserScale => _laserScale;
        ...
        private void OnDrawGizmos()
        {
            ...
            Gizmos.DrawWireSphere(transform.position + transform.forward * pruebahit.distance, _radiusCollider);
        }
        ...
        void Update()
        {
            ...
            if (_laserPS != null && _updateSaver == false)
            {
                ...
                RaycastHit hit;
                if (Physics.SphereCast(transform.position, _radiusCollider, transform.forward, out hit, _maxLength, _layerMask))
                {
                    pruebahit = hit;
        ...
        public void SetLaserScale(float laserScale)
        {
            _laserScale = laserScale;
            _laserMat.SetFloat("_Scale", _laserScale);
        }
        ...
        public void SetMaxLenght(float value)
        {
            _maxLength = value;
        }
        ...
        {
            ObjectPoolBossFireLaser.Instance.ReturnToPool(this);
        }
