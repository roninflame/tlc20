...
namespace PolloScripts
{
    public class ObjectTracker : MonoBehaviour
    {
        ...
        public Vector3 GetProjectedPosition(float fTime, Vector3 offset)
        {
            Vector3 v3Ret = new Vector3();

            //X0 + v0 * t + 1/2 a t^2
            v3Ret = goTrackingObject.transform.position + offset + (v3AverageVelocity * Time.deltaTime * (fTime / Time.deltaTime)) + (0.5f * v3AverageAcceleration * Time.deltaTime * Mathf.Pow(fTime / Time.deltaTime, 2));
            if (goIndicator != null)
                goIndicator.transform.position = v3Ret;
